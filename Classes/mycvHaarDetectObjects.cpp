   ...
   // +++lal
    ...
    if( !cascade )
       // CV_ERROR( CV_StsNullPtr, "Hidden cascade has not been created.\n"
		...
		printf("Hidden cascade has not been created.. Use cvSetImagesForHaarClassifierCascade\n");
	...
	int pq0 = cascade->pq0[pq_offset];
	int pq1 = cascade->pq1[pq_offset];
	int pq2 = cascade->pq2[pq_offset];
	int pq3 = cascade->pq3[pq_offset];
					...
					int min_neighbors, int flags, CvSize min_size )
{
    ...
    if( !CV_IS_HAAR_CLASSIFIER(cascade) ) {
      //  CV_ERROR( !cascade ? CV_StsNullPtr : CV_StsBadArg, "Invalid classifier cascade" );
		printf("Invlid classifier cascade\n");
		return NULL;
	}
    if( !storage ) {
    ...
    if( !cascade->hid_cascade ) {
		...
		myicvCreateHidHaarClassifierCascade(cascade);
		if( cvGetErrStatus() < 0 )  
		{
			return NULL;
		}
         ...
         //  CV_CALL( temp_storage_thread = cvCreateMemStorage(0));
           ...
           // CV_CALL( seq_thread[i] = cvCreateSeq( 0, sizeof(CvSeq),
			...
			seq_thread[i] = cvCreateSeq( 0, sizeof(CvSeq),
										sizeof(CvRect), temp_storage_thread );
			if( cvGetErrStatus() < 0 ) {                                     
				printf("Inner function failed. --2" );   
				return NULL;
			}
        }
    else
