...
//	if((!nose) || (!plane) || (!cylinder)) return;
	...
	if(fav.totNoses) {
		...
		position.x = glView.frame.size.width - rNose[0]->x - rNose[0]->width;
		position.y = glView.frame.size.height - rNose[0]->y - rNose[0]->height;

		NSLog(@"nose-d0:%.0f %.0f %.0f %.0f %.0f %.0f",glView.frame.size.width, glView.frame.size.height,
			  position.x,position.y,(float)(rNose[0]->width),(float)(rNose[0]->height));

		position.x -=  (glView.frame.size.width/2);
		position.y -=  (glView.frame.size.height/2) ;

		//position.z = -(float)(rNose[0]->width);
		NSLog(@"nose-d1:%.0f %.0f %.0f %.0f %.0f %.0f",glView.frame.size.width, glView.frame.size.height,
			  position.x,position.y,(float)(rNose[0]->width),(float)(rNose[0]->height));
		
		nose.currentPosition = position;
	}
	
	
	if(fav.totMouths) {
		
		position.x = glView.frame.size.width - rMouth[0]->x - rMouth[0]->width;
		position.y = glView.frame.size.height - rMouth[0]->y - rMouth[0]->height;
		
		NSLog(@"cylinder-d:%.0f %.0f %.0f %.0f %.0f %.0f",glView.frame.size.width, glView.frame.size.height,
			  position.x,position.y,(float)(rMouth[0]->width),(float)(rMouth[0]->height));
		position.x -=  (glView.frame.size.width/2);
		position.y -=  (glView.frame.size.height/2) ;
		
		//position.z = -(float)(rMouth[0]->width);
		NSLog(@"cylinder-d:%.0f %.0f %.0f %.0f %.0f %.0f",glView.frame.size.width, glView.frame.size.height,
			  position.x,position.y,(float)(rMouth[0]->width),(float)(rMouth[0]->height));
		
		//cylinder.currentPosition = position;
		//mouth.currentPosition = position;
	...
	if(fav.totEyes) {
		...
		position.x = glView.frame.size.width - rEyes[0]->x - rEyes[0]->width;
		position.y = glView.frame.size.height - rEyes[0]->y - rEyes[0]->height;
		
		NSLog(@"cone-d:%.0f %.0f %.0f %.0f %.0f %.0f",glView.frame.size.width, glView.frame.size.height,
			  position.x,position.y,(float)(rEyes[0]->width),(float)(rEyes[0]->height));

		position.x -=  (glView.frame.size.width/2);
		position.y -=  (glView.frame.size.height/2) ;
		
		//position.z = -(float)(rEyes[0]->width);
		NSLog(@"cone-d:%.0f %.0f %.0f %.0f %.0f %.0f",glView.frame.size.width, glView.frame.size.height,
			  position.x,position.y,(float)(rEyes[0]->width),(float)(rEyes[0]->height));
		
		//cone.currentPosition = position;
	}
