...
+ (UIImage *)imageWithCVImage:(IplImage *)cvImage {
    ...
    int stride = width;
#ifdef MALLOC_DEBUG
	// +++lal: added factor of 2
	UInt8 *pixelData = (UInt8 *) NTIMalloc((width * height), (char *)__FILE__, __LINE__);
#else
...
- (CGContextRef)createARGBBitmapContext {
	...
	bitmapData = NTIMalloc(bitmapByteCount, (char *)__FILE__, __LINE__);
#else
    ...
    if (context == NULL) {
#ifdef MALLOC_DEBUG
		NTIFree(bitmapData, (char *)__FILE__, __LINE__);
#else
        free (bitmapData);
#endif
...
- (IplImage *)cvImage {
    ...
    if (data) {
#ifdef MALLOC_DEBUG
		NTIFree(data, (char *)__FILE__, __LINE__);
#else
        free(data);
#endif
...
- (IplImage *)cvGrayscaleImageMY {
    ...
    // where any drawing to the bitmap context will be rendered.
#ifdef MALLOC_DEBUG
	bitmapData = NTIMalloc(bitmapByteCount, __FILE__, __LINE__);
#else
    bitmapData = malloc(bitmapByteCount);
#endif
    if (bitmapData == NULL) {
